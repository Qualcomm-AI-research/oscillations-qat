...
# Bash script for running QAT MobileNetV2 training configuration.
...
# $ ./bash/train_mobilenetv2.sh --IMAGES_DIR path_to_imagenet_raw --MODEL_DIR path_to_weights
#
...
CMD_ARGS='--architecture mobilenet_v2_quantized
...
if [ $METHOD == "freeze" ]; then
  CMD_QAT='--oscillations-freeze-threshold 0.1'
  if [ $N_BITS == 3 ]; then
    CMD_BITS='--n-bits 3 --learning-rate 0.01 --weight-decay 2.5e-05 --oscillations-freeze-threshold-final 0.011'
  else
    CMD_BITS='--n-bits 4 --learning-rate 0.0033 --weight-decay 5e-05 --oscillations-freeze-threshold-final 0.01'
  fi
else
  CMD_QAT='--oscillations-dampen-weight 0 --oscillations-dampen-weight-final 0.1 --weight-decay 2.5e-05'
  if [ $N_BITS == 3 ]; then
    CMD_BITS='--n-bits 3 --learning-rate 0.01'
  else
    CMD_BITS='--n-bits 4 --learning-rate 0.0033'
  fi
fi
